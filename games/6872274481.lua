...
local function addBlur(parent)
	...
	blur.Image = getcustomasset('FlowVape/assets/new/blur.png')
...
run(function()
	...
	vape.Components.HotbarList = function(optionsettings, children, api)
		...
		textbuttonicon.Image = getcustomasset('FlowVape/assets/new/add.png')
			...
			local mounted = bedwars.Roact.mount(create('BillboardGui', {
				...
				create('Frame', {
					...
					create('ImageLabel', {
						...
						BackgroundTransparency = 1,
						Image = getcustomasset('FlowVape/assets/new/blur.png'),
...
run(function()
    ...
    local function hookViewmodel()
        ...
        local function hookVM(vm)
            ...
            connections[#connections + 1] = vm.ChildAdded:Connect(tryApply)
        end
        local vm = cam:FindFirstChild("Viewmodel")
        if vm then hookVM(vm) end
        connections[#connections + 1] = cam.ChildAdded:Connect(function(child)
    ...
    local function hookContainer(container)
        ...
        connections[#connections + 1] = container.ChildAdded:Connect(tryApply)
    end
    ...
    local function hookCannonViewmodel()
        ...
        local function hookVM(vm)
            ...
            connections[#connections + 1] = vm.ChildAdded:Connect(function(child)
                if child.Name == CANNON_TOOL_NAME then
                    task.wait()
                    attachCannonReskin(child, CFrame.identity, CANNON_HAND_SCALE)
                end
    ...
    local function hookCannonContainer(container)
        ...
        connections[#connections + 1] = container.ChildAdded:Connect(function(child)
            if child.Name == CANNON_TOOL_NAME then
                task.wait()
                attachCannonReskin(child, CFrame.identity, CANNON_HAND_SCALE)
            end
    ...
    local function hookCannonBlocksFolder(blocksFolder)
        ...
        connections[#connections + 1] = blocksFolder.ChildAdded:Connect(function(child)
            if child.Name == CANNON_TOOL_NAME then
                task.wait()
                attachCannonReskin(child, CANNON_PLACED_OFFSET, 1)
            end
    ...
    local function hookAllWorldCannons()
        ...
        connections[#connections + 1] = worlds.ChildAdded:Connect(function(world)
            task.wait()
            local blocks = world:FindFirstChild("Blocks")
            if blocks then hookCannonBlocksFolder(blocks) end
        end)
    ...
    local function stopSkinChanger()
        ...
        pcall(removeKitSkinHook)
        pcall(clearStoreSkins)
        pcall(unhookCannonSounds)
...
run(function()
    local InfiniteFly
    local InfiniteFlyMode
    local InfiniteFlySpeed
    local InfiniteFlyVerticalSpeed
    ...
    local function disablefunc()
        if flyoffConnection then
            flyoffConnection:Disconnect()
            flyoffConnection = nil
        end
        if bodyvelo then
            bodyvelo:Destroy()
            bodyvelo = nil
        ...
        pcall(function()
            local oldclonepos = clone and clone.Position.Y or oldcloneroot.Position.Y
    ...
    InfiniteFly = vape.Categories.Blatant:CreateModule({
        ...
        Function = function(callback)
            if callback then
                ...
                if entitylib.isAlive and entitylib.character.Humanoid.Health > 0 and isnetworkowner(entitylib.character.HumanoidRootPart) then
                ...
                local goneup = false
                oldcloneroot.Velocity = oldcloneroot.Velocity + (entitylib.character.Humanoid.FloorMaterial ~= Enum.Material.Air and Vector3.new(0, 600, 0) or Vector3.zero)
                InfiniteFly:Clean(runService.Heartbeat:Connect(function(delta)
                    if LobbyCheck.Enabled and store.matchState == 0 then return end
                    ...
                    if entitylib.isAlive then
                        if isnetworkowner(oldcloneroot) then
                            ...
                            local flyVelocity = entitylib.character.Humanoid.MoveDirection * (InfiniteFlyMode.Value == 'Normal' and InfiniteFlySpeed.Value or 20)
                            entitylib.character.HumanoidRootPart.Velocity = flyVelocity + (Vector3.new(0, playerMass + (InfiniteFlyUp and InfiniteFlyVerticalSpeed.Value or 0) + (InfiniteFlyDown and -InfiniteFlyVerticalSpeed.Value or 0), 0))
                            if InfiniteFlyMode.Value ~= 'Normal' then
                                entitylib.character.HumanoidRootPart.CFrame = entitylib.character.HumanoidRootPart.CFrame + (entitylib.character.Humanoid.MoveDirection * ((InfiniteFlySpeed.Value + getSpeed()) - 20)) * delta
                            end
                ...
                if clonesuccess and oldcloneroot and clone and lplr.Character.Parent == workspace and oldcloneroot.Parent ~= nil and disabledproper and cloned == lplr.Character then
                    ...
                    origcf[2] = ray and ray.Position.Y + (entitylib.character.Humanoid.HipHeight + (oldcloneroot.Size.Y / 2)) or clone.CFrame.p.Y
                    ...
                    flyoffConnection = runService.Heartbeat:Connect(function()
                        if oldcloneroot and clone and clone.Parent then
                            oldcloneroot.Velocity = Vector3.new(clone.Velocity.X, -1, clone.Velocity.Z)
    ...
    LobbyCheck = InfiniteFly:CreateToggle({
        Name = 'Lobby Check',
        Default = true
    })
