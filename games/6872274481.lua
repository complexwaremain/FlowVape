...
local function characterAdded(char)
    ...
    -- Kill existing connections on CFrame and Velocity
    ...
    -- Hook the RootPart:SetPrimaryPartCFrame method if used by anticheat
    local originalSetCFrame
    originalSetCFrame = hookmetamethod(rootPart, "__index", function(self, key)
        if self == rootPart and key == 'CFrame' then
            -- return a cached CFrame to bypass checks
        end
        return originalSetCFrame(self, key)
    end)

    -- Kill Heartbeat-based position validation
    for _, v in getconnections(game:GetService('RunService').Heartbeat) do
        local funcName = tostring(v.Function)
        if funcName:match('anticheat') or funcName:match('AC') or funcName:match('lagback') or funcName:match('flag') then
            pcall(function()
                hookfunction(v.Function, function() end)
            end)
        end
    ...
    -- Hook firetouch/teleport detection via Humanoid
    ...
    -- Hook the RemoteEvent that the server uses to validate movement
    local player = game:GetService('Players').LocalPlayer
    for _, conn in getconnections(player.Idled) do
        pcall(function()
...
Disabler = vape.Categories.Utility:CreateModule({
    ...
    Tooltip = 'Disables server-side movement anticheat (CFrame, Velocity, WalkSpeed, Heartbeat checks)'
})
...
runFunction(function()
    ...
    local function updateRotations()
        if vapeLookAtPosition then
            local newposition = Vector3.new(vapeLookAtPosition.X, vapeOriginalRoot.Position.Y, vapeLookAtPosition.Z)
