...
run(function()
	...
	local function clean()
			...
			if char then
				for _, v in char:GetDescendants() do
					if v:IsA('ParticleEmitter') or v:IsA('Trail') or v:IsA('Beam') then
						if v.Parent and not v.Parent:IsA('Tool') then
							v:Clear()
						end
					end
				end
		...
		pcall(function()
			...
			for _, v in sounds do
				if v:IsA('Sound') and not v.IsPlaying and not v.Looped then
		...
		pcall(function()
			for _, v in workspace:GetDescendants() do
				if v:IsA('BasePart') and v.Name == 'Effect' and not v.Anchored then
					pcall(function() v:Destroy() end)
				end
			end
		end)
		...
		pcall(function()
			...
			if gui then
				for _, v in gui:GetDescendants() do
					if v:IsA('ImageLabel') and v.Visible == false and v.Parent and v.Parent.Visible == false then
						pcall(function() v:Destroy() end)
					end
				end
			end
	...
	MemoryFixer = vape.Categories.Utility:CreateModule({
		Name    = 'MemoryFixer',
		Tooltip = 'Periodically cleans up memory leaks, dead sounds, effects and particles.',
		Function = function(callback)
			if callback then
				clean()
	...
	Interval = MemoryFixer:CreateSlider({
		...
		Default = 30,
		Suffix  = 's',
		Tooltip = 'How often to run the memory cleanup in seconds.'
	})

	MemoryFixer:CreateButton({
		Name     = 'Clean Now',
		Tooltip  = 'Runs the cleanup immediately.',
		Function = function()
			clean()
			vape:CreateNotification('MemoryFixer', 'Memory cleaned!', 2)
		end
	})
end)
